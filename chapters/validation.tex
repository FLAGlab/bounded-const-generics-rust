...
The \inrust{Vector} type on \ref{lst:vector_vanilla} has the size of the vector
...
method on \ref{lst:vector_vanilla} adds the new element to the \inrust{inner}
...
There is also a new method \inrust{from_vec_unchecked}, this method panics
during execution if the size of the \inrust{vec} argument is not \inrust{N}.
This is necessary because, there is no way to do this verification during
compilation, as a consequence of the dynamic length of the \inrust{Vec} type.
It is responsibility of the author of the \inrust{Vector<N>} type to use
