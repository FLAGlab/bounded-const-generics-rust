...
\textsc{mir} and \textsc{sir} generated can be found on Listings
\ref{lst:mir_sire_example} and \ref{lst:sir_sire_example} respectively. Even
...
predicate is satisfiable or not. For example, is possible to encode a root
finding problem as a function equality problem, these kind of problems do not
have an analytic solution for high degree polynomials.
...
what makes possible to write Rust code with almost no type annotations. Rust
compiler does type inference by adding constrains over inference variables and
then trying to bind each variable to a concrete type.
...
can be reduced over an equality constrain over its constant parameters, i.e. as
a constrain of equality between functions, which can be represented and solved
using \textsc{sire}. 
